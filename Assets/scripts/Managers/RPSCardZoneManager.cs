...
public abstract class RPSCardZoneManager<T> : MonoBehaviour, IPointerEnterHandler where T : RPSCardModel
{
    [Header("--------------Prefabs----------------")]
    ...
    /// <returns>A <see cref="RPSCardGroupManager{T}"/> representing the newly added group.</returns>
    /// <remarks>
    ...
    /// </remarks>
    public RPSCardGroupManager<T> AddGroup(List<T> cards = null)
    {
        ...
        if (cards != null)
        {
            ...
            {
                cardGroup.GetComponent<RPSCardGroupManager<T>>().AddCard(card);
            }
        }
        return cardGroup.GetComponent<RPSCardGroupManager<T>>();
    }
    ...
    public void RefreshCardZone()
    {
        ...
        foreach (Transform child in transform)
        {
            ...
            RPSCardGroupManager<T> cardGroup = child.GetComponent<RPSCardGroupManager<T>>();
    ...
    /// </returns>
    public virtual List<RPSCardManager<T>> GetAllCards()
    {
        List<RPSCardManager<T>> cards = new List<RPSCardManager<T>>();
        foreach (Transform groupTransform in transform)
        {
            foreach (Transform cardTransform in groupTransform)
            {
                cards.Add(cardTransform.gameObject.GetComponent<RPSCardManager<T>>());
            }
        }
        return cards;
    }
    ...
    /// A list of <see cref="CardManager{T}"/> objects representing all the cards in the card zone.
    /// </returns>
    public virtual List<RPSCardManager<T>> GetAllSelectedCards()
    {
        List<RPSCardManager<T>> selectedCards = new List<RPSCardManager<T>>();
        foreach (Transform groupTransform in transform)
        {
            foreach (Transform cardTransform in groupTransform)
            {
                RPSCardManager<T> card = cardTransform.gameObject.GetComponent<RPSCardManager<T>>();
                if (card.isSelected)
                {
    ...
    /// </remarks>
    /// <returns>
    /// A list of <see cref="RPSCardGroupManager{T}"/> objects representing all the cards in the card zone.
    /// </returns>
    public RPSCardGroupManager<T> GroupSelectedCards()
    {
        List<RPSCardManager<T>> selectedCards = new List<RPSCardManager<T>>();
        foreach (RPSCardManager<T> card in GetAllCards())
        {
            if (card.isSelected)
            {
                selectedCards.Add(card);
        ...
        GameObject cardGroup = Instantiate(cardGroupPrefab, transform);
        foreach (RPSCardManager<T> card in selectedCards)
        {
    ...
    public void OnPointerEnter(PointerEventData eventData)
    {
        ...
        {
            RPSCardManager<T> cardManager = eventData.pointerDrag.GetComponent<RPSCardManager<T>>();
            if (cardManager && cardManager.label == this.label)
            {
                if (transform.childCount <= 0)
                {
                    RPSCardGroupManager<T> groupManager = AddGroup();
