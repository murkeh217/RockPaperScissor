...
public abstract class RPSCardManager<T> : MonoBehaviour, IBeginDragHandler, IDragHandler, IEndDragHandler where T : RPSCardModel
{
    [Header("--------------Prefabs----------------")]
    ...
    public void UpdateParent(Transform newParent)
    {
        ...
        {
            oldParentGroup.parent.GetComponent<RPSCardZoneManager<T>>().RefreshCardZone();
        }
    ...
    {
        RPSEventManager<T, RPSCardManager<T>>.CARD_PICKED.Invoke(this);
    }
    ...
    {
        RPSEventManager<T, RPSCardManager<T>>.CARD_DROOPED.Invoke(this);
    }
    ...
    private IEnumerator DestroyAndWait()
    {
        ...
        transform.parent.parent.GetComponent<RPSCardZoneManager<T>>().RefreshCardZone();
    }
    #endregion
}
