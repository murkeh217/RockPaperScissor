...
public abstract class RPSCardGroupManager<T> : MonoBehaviour, IPointerEnterHandler where T : RPSCardModel
{
    [Header("--------------Prefabs----------------")]
    ...
    RPSCardZoneManager<T> parentZone;
    ...
    protected virtual void Awake()
    {
        ...
        parentZone = transform.parent.GetComponent<RPSCardZoneManager<T>>();
    }
    #endregion
    ...
    public virtual RPSCardManager<T> AddCard(T cardModel = null)
    {
        GameObject card = Instantiate(cardPrefab, transform);
        RPSCardManager<T> cardManager = card.GetComponent<RPSCardManager<T>>();
        if (cardModel != null)
        {
            cardManager.SetData(cardModel);
        }
        cardManager.label = transform.parent.GetComponent<RPSCardZoneManager<T>>().label;
        return cardManager;
    }
    ...
    public void OnPointerEnter(PointerEventData eventData)
    {
        if (eventData.pointerDrag)
        {
            ...
            if (cardManager.label == transform.parent.GetComponent<RPSCardZoneManager<T>>().label)
            {
                eventData.pointerDrag.GetComponent<RPSCardManager<T>>().UpdateParent(this.transform);
            }
        }
    }
    #endregion
